...
import EngineMakePopup from './MakePopup';
 ...
 * Admin Engine Makes
 */
class EngineMakes extends Component {
  ...
  render() {
    ...
    return (
      <div className="EngineMakes">
        <Filters
          create={{
            action: createEngineMake,
            caption: '+ Add engine make',
            onSave: this.query,
            popup: EngineMakePopup,
            title: 'Add engine make'
          }}
          route={route}
        />
                ...
                <>
                  <tr className={clsx({ odd, selected })}>
                    ...
                    <td className="no-padding">
                      <EngineMakePopup
                        action={updateEngineMake}
                        data={make}
                        title="Edit engine make"
                        onSave={this.query}>
                        {name}
                        <Icon value="fal-edit" />
                      </EngineMakePopup>
                    ...
                    <td className="delete">
                      <Confirm
                        message="Are you sure you want to delete this engine make?"
                        ok={{ title: 'Delete Engine Make' }}
   ...
   */
  delete(_id) {
    return when(deleteEngineMake(_id)).then(() => () => {
      this.query();
    });
  }
  ...
  query(props) {
    ...
    if (isUndefined(props) || !isEqual(query, props.route.query)) {
      this.props.dispatch(getEngineMakes(query));
    }
  }
}
